+++
ShowToc = true
TocOpen = false
aliases = ["beepbox", "beep box", "music"]
author = ""
categories = ["Creations"]
description = "A bunch of silly little tunes."
series = []
tags = ["Music"]
title = "BeepBox Creations"
comments = false
disableHLJS = true
ShowReadingTime = false
+++
The chiptune tunes in this post were created using the free online tool for sketching and sharing instrumental music, 
[BeepBox by John Nesky](https://www.beepbox.co). Please consider supporting them by donating through the link on their site!

# Something About Us (by Daft Punk)

{{<audio src="/audio/somethingaboutus.wav" caption="Chiptune cover of Something About Us" cover="/audio/somethingaboutus.png">}}

One of my favorite songs from their 2001 album "Discovery". This tune is played on piano and can be heard throughout the 
whole song. You can listen to it [here](https://open.spotify.com/track/1NeLwFETswx8Fzxl2AFl91?si=124d258dd9174894).